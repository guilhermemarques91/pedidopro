...
final class CatalogController
{
    ...
    {
        $tree = MenuSyncService::localTree();
    ...
    {
        $channel = self::channel($req->intParam('channelId'));
    ...
    {
        $id = $req->intParam('id');
        self::exists('menu_categories', $id, 'Categoria');
        $in = $req->input();
        $fields = [];
        $values = [];
        if ($in->has('name')) {
    ...
    private static function saveItem(Request $req, ?int $id): void
    {
        $in = $req->input();
        if ($id !== null) {
            self::exists('menu_items', $id, 'Item');
        }
        $categoryId = $in->integer('category_id', $id === null);
        if ($categoryId !== null) {
            self::exists('menu_categories', $categoryId, 'Categoria');
        }
    ...
    {
        Http::json(MenuSyncService::publish(self::channel($req->intParam('channelId'))));
    }

    /** POST /delivery/menu/import/:channelId — importa o cardápio remoto (bootstrap). */
    public static function import(Request $req): void
    {
        Http::json(MenuSyncService::import(self::channel($req->intParam('channelId'))));
    }
    ...
    private static function channel(int $id): array
    {
    ...
    private static function exists(string $table, int $id, string $label): void
    {
        if (!Db::queryOne("SELECT id FROM {$table} WHERE id = ?", [$id])) {
