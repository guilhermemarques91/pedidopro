...
// O backfill é escopado por organização (multi-tenant do ERP): roda o lote para cada
// org ativa, para que o cron cubra todos os tenants sem precisar de um job por org.
foreach (Db::query('SELECT id, name FROM organizations WHERE active = 1 ORDER BY id') as $org) {
    $orgId = (int) $org['id'];
    try {
        $result = GeoService::backfill($limit, $orgId);
        echo '[' . date('Y-m-d H:i:s') . "] org={$orgId} geocoded={$result['geocoded']} reverse_geocoded={$result['reverse_geocoded']} remaining={$result['remaining']}\n";
    } catch (\Throwable $e) {
        echo '[' . date('Y-m-d H:i:s') . "] org={$orgId} ERRO: " . $e->getMessage() . "\n";
    }
}
