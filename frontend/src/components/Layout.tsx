...
type NavItem = { to: string; label: string; icon: typeof LayoutDashboard; end?: boolean; roles?: UserRole[] };
type NavGroup = { title?: string; items: NavItem[] };

// `roles` ausente = visível a todos os papéis autenticados.
// Menu agrupado por área: Delivery (vendas), Compras, Cadastros, Integrações, Admin.
...
export function Layout() {
  ...
  const visibleGroups = navGroups
    .map((g) => ({ ...g, items: g.items.filter((n) => !n.roles || (user && n.roles.includes(user.role))) }))
        ...
        <nav className="flex-1 space-y-4 overflow-y-auto px-3 pb-4">
          {visibleGroups.map((group, gi) => (
            <div key={group.title ?? `g${gi}`} className="space-y-1">
              {group.title && (
                <p className="px-3 pb-1 pt-2 text-xs font-semibold uppercase tracking-wide text-slate-400">{group.title}</p>
              )}
